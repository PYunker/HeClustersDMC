program dmc
    ...
    use system ! zde je definovan potencial
    ! ================= metaparametry =====================
    implicit none
    ...
    read(*,*) alfa
    write(*,'(a14)',advance='no') "x0 : " ! pocatecni poloha walkeru (do budoucna by ji bylo dobre presunout do modulu 'system')
    read(*,*) x0
    PRINT*, "==================================="
    ! =====================================================

    call simulace(p_bloku, p_kroku, N0, N_max, N_min, alfa, dt, x0, Er) ! samotna simulace
    contains ! =============== SUBRUTINY ==================
    
    subroutine simulace(p_bloku, p_kroku, N0, N_max, N_min, alfa, dt, x0, Er)
        implicit none
        ...
        do i_blok = 1,p_bloku ! simulace probiha v blocich, ve kterych se sbiraji ruzne statistiky (zatim jen potencial a Er)
            pot = 0
            do i_krok = 1,p_kroku
                call difuze(rep, m, I_0, pra, p_0, pe, h, dp, Er, dt, ub) ! upravi polohu replik (a)
    ...
    subroutine difuze(rep, m, I_0, pra, p_0, pe, h, dp, Er, dt, ub)
        implicit none
        ...
        do j = 1,h ! v pouzivanem rozsahu
            if (m(j) /= 0) then ! pro vesechny zive repliky
                ...
                pot_e = potencial([rep(j,:)]) ! vepocte potencial danne repliky
                pe = pe + pot_e
    ...
    subroutine vetveni(rep, m, I_0, pra, p_0, h, Er, alfa, p_replik, ub)
        ...
        do j = 1,h
            ...
            elseif (m(j) == 3) then ! pro repliky s m = 3
                rep(nove([k,k+1]),:) = rep([j,j],:) ! vytvori  kopie
                m(nove([k,k+1])) = [1,1]
                k = k + 2
            endif
        end do
