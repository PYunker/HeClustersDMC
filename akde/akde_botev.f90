...
contains
    ...
    subroutine botev_get_estimate(d, gam, n, lam, process, X, grid, &
        ...
        type(c_ptr) :: baseptr
        integer :: i, j, k, i2, idx(100) = [(i, i=1,100)], max_iter = 1000!, &
                   !seed(4)
        ...
        ! DEEEEEEEEEEEEBUG
        write(*,"('ALLOCATED(mu) : ', L1, ' (71)')") associated(mu)
        ...
        ! DEEEEEEEEEEEEEBUG
        write(*,"('ALLOCATED(mu) : ', L1, ' (98)')") associated(mu)

        ! initialization of random seed, not needed in this version
        ...
        do
            ...
            do i=1,size(lb_idx)
                i2 = lb_idx(i)
                call dpotrf('L', d, sig(:,:,i2), d, j)
                do j=1,d
                    det(i2) = det(i2) * sig(j,j,i2)
                end do
                call dtrtri('L','N',d,sig(:,:,i2),d,j)
                !stopa(i2) = dlantr('F','L','N',d,d,sig(:,:,i2),d,curv)**2
                stopa(i2) = norma(sig(:,:,i2))
            end do
        ...
        do i=1,size(lb_idx)
            ...
            call dtrmm('L','L','N','N',d,grid_n,1d0,sig(:,:,i2),d,mesh,d)
            temp = -0.5d0 * norm2(mesh,dim=1)**2d0
            estimate = estimate + w(i2) * norm_const / det(i2) * exp(temp)
        end do
        ...
        write(*,"('ALLOCATED(mu) : ', L1, ' (255)')") associated(mu)
        ...
        deallocate(stopa)
        deallocate(det)
        write(*,"('ALLOCATED(mu) : ', L1, ' (264)')") associated(mu)
        
        call MPI_WIN_FREE(shared_win)
