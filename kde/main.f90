program akde_mse_test
    ...
    integer, parameter :: DP = KIND(0.0D0), DP_SIZE = sizeof(0.0D0), &
						  INT_SIZE = sizeof(1), &
						  d = 1, sample_size = 1000, grid_n = 150, gam = 500
    integer :: world_size, world_rank, run, node_runs, runs = 2
    real(kind=DP) :: Ef(grid_n) = 0, Ef2(grid_n) = 0, estimate(grid_n), &
                     Es(grid_n) = 0, Es2(grid_n) = 0
    real(kind=DP) :: grid(d,grid_n), h
...
!============= utilities converted to subroutines for convenience =============
    ...
    subroutine mpi_setup()
		...
		! allocate variables of the estimator
		!allocate(estimator%mu(d, gam))
		!allocate(estimator%w(gam))
		!allocate(estimator%sig(d, d, gam))
		!allocate(estimator%det(gam))
	...
	subroutine cleanup()
		...
		!deallocate(estimator%mu)
		!deallocate(estimator%w)
		!deallocate(estimator%sig)
		!deallocate(estimator%det)
