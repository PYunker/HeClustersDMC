...
module kde
    ...
    public :: botev_get_estimate, win_allocate, process_type, dp_size, neco
...
contains
    ...
    subroutine botev_get_estimate(d, gam, n, lam, process, X, grid, &
        ...
        ! deallocate shared memory to prevent leaks (about this i'm not too sure)
        !if (process%node_rank == 0) then
        !    deallocate(mu)
        !    deallocate(w)
        !    deallocate(sig)
    ...
    subroutine kde_get_estimate(d, n, X, process, grid, &
                                grid_n, estimate, gam, bandwidth)
        integer, intent(in) :: d, n, grid_n
        integer, optional, intent(in) :: gam
        type(process_type), intent(in) :: process
        real(kind=DP), intent(in) :: grid(d,grid_n), X(d,process%n_proc)
    ...
    subroutine win_allocate(numel, btsize, process, window, baseptr)
        ...
        ! does it have to be intent(inout) ? not sure
